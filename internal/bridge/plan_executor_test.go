...
func bridgeTestConfig() model.Config {
	return model.Config{
		...
		// Match production template default (worktree.enabled=true).
		// shouldInsertSystemCommit injects __system_commit whenever worktree
		// mode is off, so leaving the zero value here would silently grow
		// every plan in this package by an extra task.
