...
// (Removed) CommitPolicyConfig tests — schema retired. The runtime never
// enforced these gates after the orchestrator-owns-commit redesign and
// keeping the type around only invited operators to re-introduce a
// guard rail that would silently fail otherwise valid commits.
