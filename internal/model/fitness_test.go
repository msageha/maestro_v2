...
func TestSelectWinner_Empty(t *testing.T) {
	idx, tie := SelectWinner(nil, DefaultFitnessThresholds())
	if idx != -1 || tie {
		t.Errorf("empty: got idx=%d, tie=%v, want idx=-1, tie=false", idx, tie)
	}
}

func TestSelectWinner_Single(t *testing.T) {
	candidates := []FitnessScore{{Passed: true}}
	idx, tie := SelectWinner(candidates, DefaultFitnessThresholds())
	if idx != 0 || tie {
		t.Errorf("single: got idx=%d, tie=%v, want idx=0, tie=false", idx, tie)
	}
}

func TestSelectWinner_ClearWinner(t *testing.T) {
	th := DefaultFitnessThresholds()
	candidates := []FitnessScore{
		{Passed: true, RepairCount: 3, DiffLinesChanged: 100, ExecutionTime: 60 * time.Second},
		{Passed: true, RepairCount: 1, DiffLinesChanged: 50, ExecutionTime: 30 * time.Second},
		{Passed: false, RepairCount: 0, DiffLinesChanged: 10, ExecutionTime: 5 * time.Second},
	}
	idx, tie := SelectWinner(candidates, th)
	if idx != 1 || tie {
		t.Errorf("clear winner: got idx=%d, tie=%v, want idx=1, tie=false", idx, tie)
	}
}

func TestSelectWinner_Tie(t *testing.T) {
	th := DefaultFitnessThresholds()
	a := FitnessScore{Passed: true, RepairCount: 1, DiffLinesChanged: 50, ExecutionTime: 10 * time.Second}
	candidates := []FitnessScore{a, a, a}
	_, tie := SelectWinner(candidates, th)
	if !tie {
		t.Error("identical candidates should tie")
	}
}

func TestSelectWinner_ThreeCandidatesMiddleWins(t *testing.T) {
	th := FitnessThresholds{}
	candidates := []FitnessScore{
		{Passed: true, RepairCount: 5},
		{Passed: true, RepairCount: 1},
		{Passed: true, RepairCount: 3},
	}
	idx, tie := SelectWinner(candidates, th)
	if idx != 1 || tie {
		t.Errorf("middle wins: got idx=%d, tie=%v, want idx=1, tie=false", idx, tie)
	}
}

func TestSelectWinner_AllFailed(t *testing.T) {
	th := FitnessThresholds{}
	candidates := []FitnessScore{
		{Passed: false, RepairCount: 3},
		{Passed: false, RepairCount: 1},
		{Passed: false, RepairCount: 5},
	}
	idx, tie := SelectWinner(candidates, th)
