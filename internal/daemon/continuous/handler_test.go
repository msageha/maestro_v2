...
func setupTestMaestroDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	for _, sub := range []string{"queue", "results", "state/commands"} {
...
func newTestContinuousHandler(maestroDir string, cfg model.Config) *Handler {
	ch := NewHandler(maestroDir, cfg, lock.NewMutexMap(), log.New(&bytes.Buffer{}, "", 0), core.LogLevelDebug)
	return ch
}
