...
// Handler handles task retry logic.
type Handler struct {
	maestroDir string
	config     model.Config
...
// NewHandler creates a new task retry handler.
func NewHandler(maestroDir string, cfg model.Config, lockMap *lock.MutexMap, logger *log.Logger, logLevel core.LogLevel) *Handler {
	return NewHandlerWithDeps(maestroDir, cfg, lockMap, logger, logLevel, core.RealClock{})
}

// NewHandlerWithDeps creates a Handler with explicit dependencies.
func NewHandlerWithDeps(maestroDir string, cfg model.Config, lockMap *lock.MutexMap, logger *log.Logger, logLevel core.LogLevel, clock core.Clock) *Handler {
	return &Handler{
...
func (h *Handler) ShouldRetryTask(task *model.Task, exitCode int, retrySafe bool) (bool, string) {
...
func (h *Handler) CreateRetryTask(originalTask *model.Task, workerID string, exitCode int) (*model.Task, error) {
	retryConfig := h.config.Retry.TaskExecution
...
func (h *Handler) RegisterRetryTaskInState(task *model.Task, commandID string) error {
...
func (h *Handler) AddRetryTaskToQueue(task *model.Task, workerID string) error {
...
func (h *Handler) AddRetryTaskToQueueLocked(task *model.Task, workerID string) error {
...
func (h *Handler) log(level core.LogLevel, format string, args ...interface{}) {
