...
func TestSelfWriteTracker_CleanStale(t *testing.T) {
	tracker := newSelfWriteTracker()

	// Inject a mix of stale and fresh entries
	tracker.mu.Lock()
	tracker.stamps["/stale1.yaml"] = writeStamp{Hash: sha256.Sum256([]byte("s1")), Deadline: time.Now().Add(-60 * time.Second)}
	tracker.stamps["/stale2.yaml"] = writeStamp{Hash: sha256.Sum256([]byte("s2")), Deadline: time.Now().Add(-30 * time.Second)}
	tracker.stamps["/fresh.yaml"] = writeStamp{Hash: sha256.Sum256([]byte("f")), Deadline: time.Now().Add(30 * time.Second)}
	tracker.mu.Unlock()
	...
	// CleanStale removes only expired entries
	tracker.CleanStale()

	if tracker.Len() != 1 {
		t.Errorf("expected 1 entry after CleanStale, got %d", tracker.Len())
	}
}

func TestSelfWriteTracker_CleanStale_Empty(t *testing.T) {
	tracker := newSelfWriteTracker()

	// CleanStale on empty tracker should not panic
	tracker.CleanStale()

	if tracker.Len() != 0 {
		t.Errorf("expected 0 entries, got %d", tracker.Len())
	}
}

func TestSelfWriteTracker_CleanStale_AllFresh(t *testing.T) {
	tracker := newSelfWriteTracker()

	tracker.mu.Lock()
	tracker.stamps["/a.yaml"] = writeStamp{Hash: sha256.Sum256([]byte("a")), Deadline: time.Now().Add(30 * time.Second)}
	tracker.stamps["/b.yaml"] = writeStamp{Hash: sha256.Sum256([]byte("b")), Deadline: time.Now().Add(60 * time.Second)}
	tracker.mu.Unlock()

	tracker.CleanStale()

	if tracker.Len() != 2 {
		t.Errorf("expected 2 entries after CleanStale (all fresh), got %d", tracker.Len())
	}
}

func TestSelfWriteTracker_CleanStale_Concurrent(t *testing.T) {
	tracker := newSelfWriteTracker()

	// Concurrent CleanStale and Record must not panic
	done := make(chan struct{})
	go func() {
		for i := 0; i < 100; i++ {
			tracker.CleanStale()
	...
	for i := 0; i < 100; i++ {
		tracker.Record("/path.yaml", selfWriteTestData{Value: "v"})
	}
	<-done
}

func TestTickerLoop_CallsCleanStale(t *testing.T) {
	d := newTestDaemon(t)

	// Inject a stale entry
	d.selfWrites.mu.Lock()
	d.selfWrites.stamps["/stale.yaml"] = writeStamp{
		Hash:     sha256.Sum256([]byte("stale")),
		Deadline: time.Now().Add(-60 * time.Second),
	}
	d.selfWrites.mu.Unlock()

	if d.selfWrites.Len() != 1 {
		t.Fatalf("expected 1 entry, got %d", d.selfWrites.Len())
	}

	// Directly call CleanStale to simulate what tickerLoop does
	d.selfWrites.CleanStale()

	if d.selfWrites.Len() != 0 {
		t.Errorf("expected 0 entries after CleanStale via daemon, got %d", d.selfWrites.Len())
	}
}
