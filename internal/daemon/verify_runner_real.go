...
// verifyPathAllowed delegates to the shared expected-paths semantics in
// pathutil (also used by the A/B candidate-selection Stage 2 metrics).
func verifyPathAllowed(file string, expectedPaths []string) bool {
	return pathutil.AllowedByExpectedPaths(file, expectedPaths)
}
