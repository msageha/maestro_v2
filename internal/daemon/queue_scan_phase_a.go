...
// stepCheckWorktreeConfigViolations — Step 0.7.3: When AutoCommit/AutoMerge are
...
// the situation so the operator can act. Independent of the P1 stall step.
func (qh *QueueHandler) stepCheckWorktreeConfigViolations(s *scanState) {
	...
	for i := range s.commands.Data.Commands {
		...
		// Skip if integration has already advanced past the unmerged window.
		...
		msg := fmt.Sprintf(
			...
			cmd.ID, autoCommit, autoMerge, elapsedMin, timeoutMin)
		// upsertPlannerSignal dedupes by (command_id, phase_id, kind), preventing
		// duplicate notifications for the same command across scans as long as
		// the prior signal has not yet been delivered/cleared.
		qh.upsertPlannerSignal(&s.signals.Data, &s.signals.Dirty, model.PlannerSignal{
