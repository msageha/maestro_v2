...
func NewQualityGateEvaluator(cfg model.Config, clock core.Clock, dl *core.DaemonLogger, getGateFn func() GateChecker) *QualityGateEvaluator {
...
func (e *QualityGateEvaluator) ShouldEvaluate() bool {
	...
	if e.config.QualityGates.SkipGates {
		e.dl.Logf(core.LogLevelInfo, "quality_gates_skipped reason=emergency_mode")
		return false
	}
	if e.getGateFn() == nil {
		e.dl.Logf(core.LogLevelDebug, "quality_gates_skipped reason=daemon_not_available")
		return false
	}
	return true
}
...
// GetEvaluation returns the stored evaluation for a task, or nil if not found.
func (e *QualityGateEvaluator) GetEvaluation(taskID string) *model.QualityGateEvaluation {
	e.evalMutex.RLock()
	defer e.evalMutex.RUnlock()
	return e.evaluations[taskID]
}

// EvaluationCount returns the number of stored evaluations.
func (e *QualityGateEvaluator) EvaluationCount() int {
	e.evalMutex.RLock()
	defer e.evalMutex.RUnlock()
	return len(e.evaluations)
}
