...
2026-04-27 の修正で、High のうち recovery 分岐は `maestro plan recover --command-id <id>` に集約し、quarantine 中の `add-task` / `add-retry-task` を Daemon API で拒否するガードを追加した。Medium/Low は `--content-file` と Worker hook の `git add -A|--all|.` 拒否を実装し、Planner 指令書も recover / content-file 優先へ更新した。Planner 指令書の全面分割、verification round counter、persona/skill 検証のような構造変更は別途段階的に扱う。
