...
| アンサンブル検証           | build/lint/test/typecheck を重み付きで集約      | `internal/daemon/verification/` | **稼働**: `extended_verification.perspective_weights` でカテゴリ別重みを指定でき、weight < 1.0 のカテゴリは advisory (失敗しても run は fail しない)。失敗 fingerprint が初見の verify コマンドは `max_auto_retries` 予算内で 1 回自動再実行され (flake 吸収)、run 末尾で重み付きスコアが集約・記録される |
