...
| 依存（任意） | なし。Codex/Gemini CLI は現行の managed role では使用不可 |
...
# managed role は claude-code runtime のみ対応
...
internal/
  ...
  daemon/                  # Daemon + 高度化機能モジュール群
    ...
    fallback/              # 縮退モード管理
    featuregate/           # 機能ゲート
    judge/                 # 候補選択ロジック（production LLM caller は未接続）
    learnings/             # エラー学習 DB（Fingerprint）
    persona/               # ペルソナ注入
    reconcile/             # 状態修復エンジン（R0-R9）
    reviewer/              # 非同期コードレビュー
    rollout/               # ロールアウト実行管理
    scheduler/             # タスクスケジューラ
...
- **UDS の信頼境界:** `.maestro/daemon.sock` はモード `0600` で作成される。すなわち、UDS 経由のすべての保護は OS のファイルパーミッションに帰着する。CLI は `MAESTRO_AGENT_ROLE` と tmux pane の `@role` から呼び出し元 role を解決し、Daemon は role 未指定リクエストを拒否する。ただし role は同一 UNIX ユーザー内の advisory signal であり、別ユーザーや同一ユーザーの悪意あるプロセスに対する認証資格情報ではない。役割分離は *honest mistakes*（Worker が誤って Orchestrator 専用コマンドを呼ぶなど）と通常の Agent 経路の逸脱防止を目的とする。
- **Runtime の技術的強制範囲:** Agent のロール制約と Worker の禁止事項は `claude-code` の `--allowedTools` / `--disallowedTools` / `PreToolUse` フックで強制する。`codex` / `gemini` には等価な強制機構がないため、現行実装では Orchestrator / Planner / Worker の全 managed role で non-claude-code runtime を拒否する。
...
現行の managed role は Claude Code runtime のみをサポートする。
...
|---------|----------|----------|
| `claude-code` | ✅ 常に有効 | — |
| `codex` | ❌ | 現行の managed role では使用不可 |
| `gemini` | ❌ | 現行の managed role では使用不可 |

#### Orchestrator / Planner ロールの制約

Orchestrator の「委譲のみ」、Planner の「計画のみ」、Worker の「作業は worktree で行い `.maestro/` 制御面へ直接触れない」というロール制約は、claude-code の `--allowedTools` / `--disallowedTools` CLI フラグと PreToolUse フックで技術的に強制している。codex と gemini にはこの強制機構が存在せず、プロンプト依存になるため、現行実装では全 managed role で codex / gemini を許可しない（config 検証時と launcher 起動時に拒否する）。

#### フォールバック動作

managed role では runtime fallback は行わず、設定時点と launcher 起動時の両方で non-claude-code runtime を拒否する。
...
UCB1 モデル選択 ──→ 最適な LLM モデルをバンディットで選択
    ↓
ランタイム検証 ──→ managed role は claude-code のみ許可
    ↓
