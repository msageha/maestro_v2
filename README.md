...
- **監査ログ**: JSONL 形式のイベント監査ログ（ローテーション対応、デーモンへの配線は未完了）
...
| **監査ログ** | `internal/events/audit_logger.go` | 監査イベントを JSONL で追記し、100MB でローテーション・アーカイブ。fsync は件数または時間間隔でバッチ実行（実装済み・デーモンへの配線は未完了） |
