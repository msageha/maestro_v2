...
| 機能                       | 概要                                            | 実装                            | 現状                                                                                                                                   |
| -------------------------- | ----------------------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 適応的モデル選択           | UCB1 バンディットで実績ベースにモデルを選択     | `internal/daemon/bandit/`       | モデル選択への配線はあるが、統計が非永続で daemon 再起動ごとに warm-up（既定 50 サンプル）がリセットされ、実運用ではほぼ静的マッピング |
...
| 自己改善                   | エラー Fingerprint DB で失敗パターンを学習      | `internal/daemon/learnings/`    | 失敗パターンの記録・永続化のみ。参照系（`Query` / `SuggestStrategy`）に production 呼び出しが無く、ループが閉じていない                |
...
`feature_profiles`（simple/standard/complex/critical）はタスク単位の複雑度プロファイルを算出・ログするが、算出結果を機能 on/off に接続する実ゲーティングは未配線である。各機能の発火は config のグローバル `enabled` フラグでのみ決まるため、プロファイルを書き換えても挙動は変わらない。
