...
> **発火条件は worktree モードで決まる（継続モードではない）**: `__system_commit` の挿入条件の正本は `shouldInsertSystemCommit(cfg) == !cfg.Worktree.Enabled`（`internal/plan/submit_parse.go`）。すなわち **worktree モード無効時のみ**挿入され、これは単発実行・継続実行のどちらにも適用される。worktree モード有効時はデーモンが merge/publish の過程で自身でコミットするため、Worker 側のコミットタスクは挿入しない（競合・冗長になるため）。継続モードは「各イテレーションで成果が git 履歴に残る」ことに依存するが、その実現手段は本タスクではなく上記の worktree モード判定に委ねられている。

### デーモンの自動挿入フロー

1. デーモンが `plan submit` を受理する際、`worktree.enabled: false`（worktree モード無効）を検出
...
| 評価に判断が困難な問題が含まれる | c: ユーザー判断が必要 → 停止（Orchestrator が停止理由を報告） |
| 全目標が達成された | d: 完了 → 停止（Orchestrator が完了を報告） |
| `max_iterations` に到達 | 強制停止（`state/continuous.yaml` に `paused_reason` 記録、Orchestrator が報告） |
| `pause_on_failure: true` かつタスク失敗 | 自動停止（Orchestrator が報告） |
...
- 停止時は `state/continuous.yaml` に `paused_reason` を記録し、Orchestrator が tmux ペイン上でユーザーに停止理由を報告する（macOS 通知は行わない）
...
- `maestro up` は起動時に queue/results/state を初期化するため、`state/continuous.yaml` も再生成され `current_iteration` は 0 に戻る（[§6 フェーズ B](06-execution-flow.md) 参照）。これは `up` のフラグではなく起動シーケンスの一部であり、旧 `--reset` フラグは廃止済み。継続中のカウンタを保ったまま再 attach したい場合は `maestro attach` を使い、`up` を再実行しない
