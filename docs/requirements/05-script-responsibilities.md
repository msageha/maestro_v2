...
全機能は単一 Go バイナリ `maestro` のサブコマンドとして提供される。ビルド・インストールは `Makefile`（`check-deps` / `build` / `install` ターゲット）が担う。
...
## 5.1 Makefile（ビルド・インストール）

**責務**: システムレベルのビルドとインストール（旧 `install.sh` の代替）

- `make check-deps`: 依存コマンドの存在チェック（`tmux`, `go`, `claude`）。不足時はインストール手順を案内して失敗
- `make build`: `go build` で単一バイナリ `maestro` をビルド
- `make install`: `check-deps` → `build` → ビルド済みバイナリを配置（実行権限付与）
- `make lint` / `make test` / `make format` / `make clean`、および `make docker-*`（コンテナ実行）も提供

> 旧 `install.sh`（Bash ブートストラップ）は廃止し、Makefile に統合した。依存チェック・ビルド・配置のセマンティクスは同等。
