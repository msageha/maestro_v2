...
- **Agent ペインのビジー判定を実行**:
  ...
  - Agent がアイドル、または `max_in_progress_min` 以上（`>=`、到達時点でタイムアウト）
    ...
    2. 次のスキャンで再配信
  - **進捗観測後の中断（progress-interrupt）の別勘定**: release 対象の epoch で pane activity（`lease_extend_pane_active`）またはビジー確定プローブによる実進捗が観測されていた場合、そのタスクは wedge ではなく「実進捗 → mid-stream 中断」と分類し、`attempts`（dispatch budget）を消費しない（当該 epoch の lease 取得時にカウントした attempts も refund され、progress-interrupt の epoch は dispatch budget を完全に消費しない。これにより attempts が上限に達した直後の progress-interrupt が resume 前に dead-letter 判定へ到達することを防ぐ）。代わりにタスクごとの `progress_interrupts` カウンタ（上限 `retry.task_progress_interrupts`、既定 6）に計上する。上限超過後は従来の attempts 会計に戻る（wall-clock の backstop は従来どおり `max_in_progress_min`）
  ...
  - busy-check 経路自体は `releaseLease` だけを呼び、release 時点でペインへは何も送らない（タスクは未確定のまま queue に残るため恒久ロストは発生しない）。ペインの実際のリセットは次回 dispatch 時に行われる: 通常の再配信は配信前に `/clear` を送り（`execClearAndDeliver`）、resume 対象の再配信は `/clear` せず継続 nudge を送る（下記）[^clear-unwired]

[^clear-unwired]: ハングセッションの実体的な復旧のうち、プロンプト残渣の破棄・ペイン再生成は別経路の blocked-pane recovery が担う。ブロック検出ペインは短時間タイムアウトでタスクを `failed` にし、Worker ペインを respawn して次回 dispatch の入力衝突を断つ。
