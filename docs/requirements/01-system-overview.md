...
状態管理の複雑性（lease + フェンシング、2 フェーズロック、reconciliation、DLQ、キャンセル伝搬、DAG 検証）を考慮し、全機能を単一の Go バイナリ `maestro` に統合する。ビルド・インストールのみ `Makefile`（`go build` ラッパー）が担う。
...
| `maestro` | 単一 Go バイナリ（デーモン + CLI） | `make install`（check-deps + `go build` + 配置） |

> ビルド・インストールは `Makefile` の `check-deps` / `build` / `install` ターゲットで行う（旧 `install.sh` は廃止）。詳細は [§5.1](05-script-responsibilities.md) 参照。
