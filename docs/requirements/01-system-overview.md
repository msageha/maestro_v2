...
| Worker       | 単純タスク実行                      | Claude Sonnet / Opus（codex / gemini も可。[REQUIREMENTS.md](REQUIREMENTS.md) §5 C-7） | 最大 8（デフォルト 4） |
...
状態管理の複雑性（lease + フェンシング、2 フェーズロック、reconciliation、DLQ、キャンセル伝搬、DAG 検証）を考慮し、全機能を単一の Go バイナリ `maestro` に統合する。ビルド・インストールのみ mise task（`mise.toml` の `[tasks.*]`、`go build` ラッパー）が担う。
...
| `maestro setup <dir> [name]`                                       | `.maestro/` ディレクトリ構造の初期化                                                                                                                                              | ワンショット |
| `maestro up [--boost] [--continuous] [--detach\|-d] [--force\|-f]` | フォーメーション起動（デーモン + tmux + Agent）+ 起動後に自動 attach（`--detach` で抑止）                                                                                         | ワンショット |
| `maestro attach`                                                   | 稼働中の maestro tmux セッションへ再接続                                                                                                                                          | ワンショット |
| `maestro down`                                                     | フォーメーション停止（デーモン + tmux セッション終了）                                                                                                                            | ワンショット |
| `maestro status [--json]`                                          | 状態表示（Agent 一覧、queue depth、メトリクス）                                                                                                                                   | ワンショット |
...
| `maestro task heartbeat [flags]`                                   | 実行中タスクの heartbeat 送信（lease 延命。fencing 検証付き）                                                                                                                     | CLI→デーモン |
| `maestro plan <subcommand> [flags]`                                | command 完了条件の状態管理（submit / complete / add-retry-task / add-task / request-cancel / rebuild / recover / unquarantine / resume-merge / retry-publish / resolve-conflict） | CLI→デーモン |
| `maestro verify write [flags]`                                     | command-scoped verify config snapshot の登録（`plan submit` の前提。S1-1）                                                                                                        | CLI→デーモン |
| `maestro worker standby [--model <model>]`                         | 待機 Worker 検索（デバッグ・可観測性用。`plan submit` が自動で Worker 割当を行うため、Planner が直接呼び出す必要はない）                                                          | CLI→デーモン |
...
| `maestro skill <list\|candidates\|approve\|reject>`                | Skill レジストリ管理（Worker が参照する skill の登録・承認）                                                                                                                      | CLI→デーモン |
| `maestro persona list`                                             | 登録済み persona の一覧表示                                                                                                                                                       | CLI→デーモン |
...
| reconciler     | 定期スキャン + R0, R0-dispatch, R0b, R1-R10 不整合修復（worktree 統合・verify・差し戻しを含む。各パターンは [§5.8 ステップ 3](05-script-responsibilities.md#58-maestro-daemon)） |
...
| `maestro`  | 単一 Go バイナリ（デーモン + CLI） | `mise run install`（check-deps + `go build` + 配置） |

> ビルド・インストールは `mise.toml` の `check-deps` / `build` / `install` タスクで行う（旧 `install.sh` / `Makefile` は廃止）。詳細は [§5.1](05-script-responsibilities.md) 参照。
> fswatch, yq, flock は不要（Go の fsnotify, YAML ライブラリ, sync.Mutex に置換）。

## 通信方式

YAML ファイルベースの単方向メッセージキュー。書き込みは **CLI / Agent → Unix ドメインソケット（`.maestro/daemon.sock`）→ daemon（唯一のライター）→ YAML 永続化** の経路で行い、変更検知は Go fsnotify (kqueue) イベント駆動。Agent が `maestro queue write` 等を呼ぶと、実際の YAML 書き込みは daemon が行う（クロスプロセスのロック複雑性を排除。[§5.4](05-script-responsibilities.md) 参照）。
...
| クラッシュリカバリ時                        | **最終整合性（reconciliation）**        | 定期スキャンの reconciliation が queue/ + results/ + state/ の不整合を R0, R0-dispatch, R0b, R1-R10 のパターンで検知・修復。per-entity のバージョンチェック付きで直列化                 |
