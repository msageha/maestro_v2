...
## Worker capability ルーティング

`bloom_level`（複雑度 → モデル）とは**直交する軸**として、タスクに capability（得意分野）を宣言して worker 選択を誘導できる。各 worker は runtime（claude-code / codex / gemini）由来の既定 capability を宣言しており、`config.yaml` の `agents.workers.capabilities` で上書きされている場合もある。

### runtime 別の既定 capability と特性

| runtime     | 既定 capability                                                         | 特性                                                                                                                    |
| ----------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| claude-code | `code_quality`, `design_review`, `instruction_following`, `run_on_main` | コード品質・設計レビュー・厳密な指示追従・長時間の自律コーディング。`run_on_main` タスクは claude-code 限定（既存制約） |
| codex       | `bulk_implementation`, `long_horizon_autonomy`, `refactor`              | 大量 PR / リファクタ / マイグレーションの量産、長時間ターミナル自律、長大なツール呼び出し連鎖の完遂                     |
| gemini      | `multimodal`, `long_context_ingest`, `research`, `cost_efficient`       | 動画/音声/画像/PDF のネイティブ入力、超長コンテキスト、低コスト、リアルタイム調査                                       |

> **注意**: 「コードは codex 一択 / claude は文章係 / gemini はデータ処理」という三分法は過度な単純化であり実態と乖離する。3 runtime とも強力なエージェント型コーディング能力を持ち重複が大きい。優劣ではなく**特性差**として扱い、迷う場合は capability を省略する（省略時は従来どおり bloom_level + 負荷分散で割り当てられる）。

### タスクへの付与方法

| フィールド               | 強度 | 挙動                                                                                                                                                      |
| ------------------------ | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `required_capabilities`  | hard | 列挙タグを**すべて**宣言する worker のみが候補になる。満たす worker が 1 台も無い場合、`plan submit` がエラーで拒否される（タグ名を見直すか要求を緩める） |
| `preferred_capabilities` | soft | 一致タグ数が多い worker を優先する加点のみ。満たす worker が無くても割り当ては成立する                                                                    |

```yaml
tasks:
  - name: "auth-design-review"
    purpose: "認証フローの設計をレビューする"
    bloom_level: 5
    required_capabilities: [
      "design_review",
      "code_quality",
    ] # 満たす worker のみに割当
    preferred_capabilities: ["instruction_following"] # 加点のみ（任意）
    # ... 他の必須フィールド（expected_paths / definition_of_abort 等）
```

**付与ガイドライン**:

- 大半のタスクでは**両方とも省略が適切**。runtime 特性が結果品質に効くと明確に判断できる場合のみ付与する
- ミスマッチが高コストな要求（動画解析 → `multimodal`、リポジトリ全体の一括リファクタ → `refactor` / `bulk_implementation`、繊細な設計レビュー → `design_review`）には `required_capabilities` を使う
- 「できれば向いている worker に」程度の希望は `preferred_capabilities` に留める（fleet 構成に依存せず必ず割り当てが成立する）
- `worker_id` pin と併用した場合は pin が優先される（capability 不一致は WARN ログのみ）
- `run_on_main: true` の claude-code 限定は capability とは独立に従来どおり強制される
- `persona_hint`（行動スタイル）とは軸が異なる: capability は「どの runtime/worker に置くか」、persona は「その worker がどう振る舞うか」
...
| `required_capabilities`  | 任意 | worker 候補を「列挙タグをすべて宣言する worker」に絞る hard フィルタ。満たす worker が無ければ submit がエラーで拒否される。省略時は capability 制約なし（従来挙動）。`worker_id` pin と併用時は pin が優先（不一致は WARN）。詳細は §「Worker capability ルーティング」参照                                                                                                                                                                                       |
| `preferred_capabilities` | 任意 | 一致タグ数が多い worker を優先する soft な加点。満たす worker が無くても割り当ては成立する。詳細は §「Worker capability ルーティング」参照                                                                                                                                                                                                                                                                                                                         |
