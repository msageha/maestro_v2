...
また、Daemon が競合解決シグナルを送る際（`kind:conflict_resolution`）は以下の形式となる:

```
[maestro] kind:conflict_resolution command_id:cmd_xxx worker_id:worker1 integration_path:/path/to/.maestro/worktrees/cmd_xxx/_integration
```
...
| `integration_path` | 競合マーカー（`<<<<<<<`/`=======`/`>>>>>>>`）が実際に存在する統合 worktree の **絶対パス** |

**⚠️ 重要: 競合マーカーは integration worktree にある**

競合を起こした worker の worktree には競合マーカーは存在しない。実際の `<<<<<<< / ======= / >>>>>>>` マーカーは統合 worktree（`integration_path`）に存在する。競合解決タスクの worker は `integration_path` のディレクトリで作業する必要がある。
...
2. **競合内容の特定**: 構造化情報 (`base`/`ours`/`theirs`/`conflict_files`/`integration_path`) から、どの worker のどのファイルが衝突したか特定する
3. **競合解決タスクの発行**: `maestro plan add-task` で競合解決タスクを発行する。**signal の `worker`/`worker_id` フィールドから競合 worker の ID を取得し、`--worker-id` で指定する**ことで、競合を起こした worker 自身に解決タスクが割り当てられる。`content` に以下を含める:
   - `integration_path` の絶対パスを明示し、**そのディレクトリで作業するよう指示する**
   - 競合ファイル一覧（`conflict_files` から取得）
   - 競合マーカーを手動編集して解消し、`git add <files>` → `git commit --no-edit`（または適切なコミットメッセージ）を実行するよう指示する
   ...
   maestro plan add-task \
     ...
     --content "統合 worktree <integration_path> に移動し、以下のファイルの競合マーカー (<<<<<<< / ======= / >>>>>>>) を手動解消する: <conflict_files>。解消後 git add <files> && git commit --no-edit を実行する。作業ディレクトリは <integration_path>（絶対パス）であり、worker 自身の worktree ではない。" \
     --acceptance-criteria "競合マーカーが消え、コンパイル成功・テストパス" \
     --bloom-level 3 \
     --persona-hint implementer \
     --worker-id <worker>
   ```

   `<worker>` は signal メッセージの `worker:` / `worker_id:` フィールドの値（例: `worker1`）をそのまま使用する。`<integration_path>` は signal の `integration_path:` フィールドの絶対パスをそのまま使用する。
