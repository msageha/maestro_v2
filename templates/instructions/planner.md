...
- `verify.yaml` は daemon が verify_pending タスクで実機実行する **唯一の Strong Signal**。ここに無いコマンドは、Worker が task 内で実行して PASS していても、Maestro 全体としては「実行された証跡がない」状態になる
- `acceptance_criteria` / `content` 内の「go vet を通すこと」表記は LLM Worker への指示でしかなく、daemon は実行しない。Worker が指示を読み飛ばしても daemon は検知できない
- 後段で merge / publish / review に進む前の最終ガードがすり抜ける
...
**Publish 失敗時の commit_failed クリア（Daemon 経由）**:
...
`resolve-conflict` は **Publish-to-base ステップが `commit_failed_workers` で塞がれている**状況の復旧コマンド。指定 worker を `CommitFailedWorkers` から外し、統合ステータスを再 publish 可能な状態に戻す（`resolve_conflict` オペレーション）。

**重要**: 通常のフェーズ内 `merge_conflict` シグナルにはこのコマンドを使わない。フェーズ merge_conflict は `plan add-task --worker-id <worker>` で解消タスクを投入し、Worker が解消後に Daemon の `AutoRecoverAfterResolution` が `resume_merge` を自動発火する流れで復旧する（誤って `resolve-conflict` を投げると `worker is not in commit_failed_workers` で拒否される）。

`--command-id`、`--phase-id`、`--worker-id` は必須。`--conflicting-files` は競合ファイルパスのヒント（繰り返し指定またはカンマ区切り、任意）。

使用例:

```
# Publish-to-base が commit_failed_workers でブロックされている場合のみ
...
# フェーズ内 merge_conflict の正規復旧フロー (こちらが多くの場合の正解)
...
**実行ロケーション**: verification タスクは原則として統合ブランチを評価対象にするので、`run_on_integration: true`（publish 済みの最終確認なら `run_on_main: true`）を必ず付与する。これを付けないと worker worktree 上で実行され、対象 worker が auto-assignment 次第で変わるため、task content に「worker3 の worktree を検証」のような worker 固有記述があるとずれが生じる。worker 個別の worktree を見たい場合のみ `worker_id` で固定し、その場合も task content の言及と `worker_id` を必ず一致させる。
...
```bash
# 方法A: stdin に直接流し込む（最推奨・一時ファイル不要）
# add-task / add-retry-task / complete はすべての長文フィールドに
# `--<flag>-file -` で stdin を受け付ける。Planner の権限境界に収まる。
echo "fix \`git log --oneline -1\` で発見された問題を修正" |
  maestro plan add-task --content-file - ...

# 複数フィールドを stdin で渡したい場合は heredoc を併用するか、
# フィールドごとにコマンド呼び出しを分ける。
cat <<'AC' | maestro plan add-task --acceptance-criteria-file - --content '...' ...
must compile
and pass tests
AC
...
# 方法D: 一時ファイル経由（stdin が使えない例外ケースのみ）
# /tmp 配下の書き込みは Planner の本来の権限範囲外。
# stdin が選べない理由を明示できる場合に限って利用する。
maestro plan add-task \
  --purpose-file /tmp/maestro-task-purpose.txt \
  --content-file /tmp/maestro-task-content.txt \
  --acceptance-criteria-file /tmp/maestro-task-ac.txt \
  ...
```
...
**過去事例 (2026-04-29)**: `package main` を含むサンプルが Worker タスクの `content` に混入したが、編集対象ファイルの実 package は `maestroconfige2e3` だった。Worker が autonomously 修正したため最終的にビルドは通ったものの、Planner の result にも「タスク指定の package main はテストと不整合のため maestroconfige2e3 を使用」と記録されており、指示と実装の乖離は Worker の自律補正に依存していた。本節の規律で Planner 側からこの種の誤誘導を断つ。
...
3. **競合解決タスクの発行**: `maestro plan add-task` で競合解決タスクを発行する。**signal の `worker`/`worker_id` フィールドから競合 worker の ID を取得し、`--worker-id` で指定する**ことで、競合を起こした worker 自身に解決タスクが割り当てられる。`content` に以下を含める:
   ...
   **必ず `--content-file -` で stdin から content を渡し、`--acceptance-criteria` は単行インライン（シングルクオート）で渡す**（インライン `--content "..."` は日本語複数行で shell quoting 事故を起こすため避ける。retest3 で再現）。

   stdin は1度しか読めないため、`--content-file -` と `--acceptance-criteria-file -` を同時指定すると最初のフラグだけが heredoc を消費し、後者は空文字列になって daemon 側で `acceptance_criteria is required` で reject される。これが retest8 でレポートされた経路上の失敗モード:

   ```bash
   # 推奨パターン: content は heredoc、acceptance_criteria はインライン
   maestro plan add-task \
     --command-id <command_id> \
   ...
   `--acceptance-criteria` 自体を複数行にしたい場合は、ファイルに書き出してから `--acceptance-criteria-file <path>` を指定する（`-` ではなく実ファイル）:

   ```bash
   # 方法B: 複数行 acceptance_criteria は一時ファイル経由
   ac_file=$(mktemp)
   cat > "$ac_file" <<'EOF_AC'
   - 競合ファイルが両側の変更を統合した内容になっている
   - go build ./... が通る
   - go test ./... が通る
   EOF_AC
   maestro plan add-task \
     ...
     --purpose "merge conflict 解決: <conflict_files>" \
     --content-file - \
     --acceptance-criteria-file "$ac_file" \
     --bloom-level 3 \
     --expected-paths <conflict_file1> [--expected-paths <conflict_file2> ...] \
     --persona-hint implementer \
     --worker-id <worker> <<'EOF_TASK'
   ...
   **失敗例（絶対に使わない）**:
   ...
   ```bash
   # ❌ stdin を 2 つのフラグから同時に読もうとする。最初の --content-file - だけが
   #    heredoc を消費し、acceptance_criteria は空文字列のまま daemon に到達する。
   #    daemon 側で "acceptance_criteria is required" になり reject される。
   maestro plan add-task --content-file - --acceptance-criteria-file - ... <<'EOF'
   ...
   EOF
   ```

   ```bash
   # ❌ インライン --content の中で日本語複数行 + バックスラッシュ継続。
   #    zsh では "command not found" / "acceptance_criteria is required" になる。
   maestro plan add-task --content "...\\
   ..." --acceptance-criteria "..." ...
   ```
...
2. **競合解決タスクの発行**: `maestro plan add-task` で統合ブランチ上の競合を解決するタスクを発行する。
   ...
   **task_merge_conflict との違い:**
   ...
   merge_conflict と同じく、長文 content は **`--content-file -` で stdin** から渡す（インライン `--content "..."` は日本語複数行で shell quoting 事故を起こす。retest3 で再現）:
...
##### `run_on_main` の投入タイミング（必読）

`run_on_main: true` の検証タスクは **integration → main の publish が完了した後** にしか実行できない。Daemon の dispatcher は dispatch 直前に worktree state を読み、integration の状態が `published` でない場合は `ErrRunOnMainBeforePublish` で非リトライ拒否する（`internal/daemon/dispatch/validate_run_on_main.go`）。これは publish 前に main を読んでも統合成果物が反映されておらず必ず false FAIL になるため。

具体的なルール:

1. `run_on_main: true` のタスクは **publish 完了通知（result-write などで integration が `published` に遷移した後）を待ってから** `plan add-task` で投入する。
2. 単一の `plan submit` 内で publish と同時に `run_on_main` 検証を投入してはならない。Dispatcher が integration `merged` のまま検証を走らせ、毎回失敗する。
3. publish 後の `run_on_main` 検証専用フェーズを別 phase（例: `verification_after_publish`）として切り出すか、integration phase が完了したことを `await_command_complete` 等で確認した後の `plan add-task` 経由で発行する。
4. integration が `quarantined` / `publish_failed` 等で publish に失敗している間も dispatch は拒否される。再 publish が成功するまで `run_on_main` タスクは投入しない。

> 違反すると: 検証は失敗で確定、Worker は補修ループに突入、Daemon retry が積み上がり command が stuck する（cmd_1777330979_d3c29242530906ac の事故再現条件）。
