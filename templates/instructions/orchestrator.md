...
`continuous.enabled: true` の場合でも、daemon は **次の iteration を自動投入しない**。daemon の役目はイテレーション・連続失敗のカウントと自動停止条件の監視のみで、次に実行するコマンドの決定はオペレーター (= ユーザー → Orchestrator) の責務である。

- `kind:command_completed` 通知を受信したら、ユーザーに結果を報告して**ターン終了**する。Orchestrator が独自に次の iteration コマンドを `maestro queue write planner` に投入してはならない。
- 次の iteration を実行するか否かはユーザーが判断する。ユーザーが新しい指示を与えた時点で、初めて Orchestrator はコマンド投入を行う。
