...
Continuous Mode における Orchestrator の責務は以下に限定される:

1. **結果の判定**: コマンド結果通知に基づき、次のアクションカテゴリ（a〜d）を決定する
2. **次コマンドの生成**: カテゴリ a/b の場合のみ、次のコマンドを `maestro queue write planner` で投入する
3. **停止判断**: カテゴリ c/d の場合、またはフェイルセーフ条件に該当する場合、自動生成を停止しユーザーに報告する

タスクの分解・Worker への割当・リトライ判断は全て Planner の責務であり、Orchestrator は関与しない。

### フェイルセーフ機構

以下の条件のいずれかに該当する場合、自動コマンド生成を即座に停止しユーザーに報告する:

| 条件 | 確認方法 | 対応 |
|------|---------|------|
| `max_iterations` 到達 | `dashboard.md` に停止が記載される | 自動生成を停止し、ユーザーに到達を報告 |
| 連続コマンド失敗 | 直前 2 回のコマンドが連続失敗 | 自動生成を停止し、ユーザーに判断を仰ぐ |
| タスク失敗による自動停止 | `dashboard.md` に停止が記載される | 自動生成を停止し、ユーザーに報告 |
| サーキットブレーカー発動 | `dashboard.md` に記載される | 自動生成を停止し、ユーザーに報告 |

次のコマンドを自動生成する前に、必ず `.maestro/dashboard.md` を確認し、上記のいずれかに該当しないことを検証する。
