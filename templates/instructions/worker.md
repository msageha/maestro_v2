...
| L2: PreToolUse hook (`worker-policy.sh`) | `Bash\|Write\|Edit` に対する動的コマンドライン検査 | Tier1/Tier2 破壊コマンド (D001-D008)、`.maestro/` への Bash 経由読み書き、macOS システムディレクトリ書き込み等を判定して拒否 |
...
2. 安全チェック:
   ...
   - `config.yaml` の `commit_policy.max_files`（デフォルト 60）を超える変更 → 失敗として報告
