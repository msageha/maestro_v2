...
以下の原則に違反する操作は実行してはならない。違反はフックで検知・ブロックされる。

- **プロジェクト作業ツリー外のファイルを変更・削除しない**
- **`rm -rf` を使用しない**（必要な場合は `realpath` でプロジェクト内であることを確認後、個別削除）
- **破壊的 git 操作を実行しない**（`push --force`, `reset --hard`, `checkout -- .`, `clean -f`）
- **`sudo` / `su` / 権限昇格を使用しない**
- **プロセス停止コマンド**（`kill`, `killall`, `pkill`）**を使用しない**
- **リモートコード実行**（`curl|bash` 等のパイプ実行）**を行わない**
- **ディスク破壊コマンド**（`mkfs`, `dd if=`, `fdisk`, `diskutil eraseDisk`）**を実行しない**
- 10 ファイル以上の削除、プロジェクト外への変更、未知 URL へのネットワーク操作 → 作業停止し `--status failed` で報告

安全な代替手段: `push --force` → `push --force-with-lease`、`reset --hard` → `stash` → `reset`、`clean -f` → `clean -n`（dry run）→ 確認後実行。

---

## Commit Task Protocol

`__system_commit` タスクを受信した場合:

1. `git status` で変更を確認（変更なし → completed で報告。30 ファイル超 → failed で報告）
2. `.gitignore` に `.env`, `*.key`, `credentials.*`, `*.pem`, `*.secret` が含まれることを確認（なければ追加）
3. `git add <file1> <file2> ...` で個別ステージング（`git add -A` / `git add .` 禁止、`.maestro/` 除外）
4. `git commit -m "[maestro] {command_id}: {purpose}"`（`git push` は絶対禁止）

Worktree モード時: `__system_commit` は配信されない（Daemon が管理）。万が一受信した場合は completed で報告する。
