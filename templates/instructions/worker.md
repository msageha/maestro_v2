...
Worktree モードでは、**通常の Worker worktree（`<commandID>/<workerID>/` 配下）で作業する場合に限り**、Worker は commit / merge / push を一切実行しない（integration worktree での publish_conflict 解決時の例外は次段落参照）。Daemon が Worker の作業結果を検出し、auto_commit および auto_merge を自動実行する仕様である。Worker は成果物を作業ディレクトリに保存し、`maestro result write` で summary と files_changed を報告するのみでよい。

ただし `working_dir` が `.maestro/worktrees/<commandID>_integration/` 配下を指す場合（`--run-on-integration` で発行された publish_conflict 解決タスク）に限り、本セクションの「commit を実行しない」原則の例外として、`§禁止される git 操作 → 例外: integration worktree (publish_conflict 解決) での git 操作` で明示された範囲の `git add` / `git commit` を実行する義務がある（Daemon は integration worktree 上の auto_commit を行わないため、Worker 自身が確定する必要がある）。`git push` は本例外でも禁止である。
...
integration worktree（publish_conflict 解決）の場合の所作は Planner 指示に従い、`git add <conflict_file>` → `git commit -m "[maestro] resolve publish conflict"` で確定したのち `maestro result write` を呼び出す。
